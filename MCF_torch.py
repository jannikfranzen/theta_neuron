...
def stationary_firing_rate(tau,sigma,mu,n_max = 100,p_max = 100,coeff = False):
    ...
    M1 = -torch.matmul(torch.inverse(B),A)
    M2 = 2*(torch.inverse(B)-I)

      
    # Setting up memory for saving the transition matrices S
    if coeff == True: 
        S_memory = torch.zeros((n_max+1,p_max,p_max),dtype=torch.cfloat).to(DEVICE)
    ...
    firing_rate = ((1+mu) - (1-mu)*torch.real(c01) + sigma*torch.real(c11))/(2*pi)
    ...
    if coeff == True:
        ...
        c = torch.zeros((n_max+1,p_max),dtype=torch.cfloat).to(DEVICE)
        c[0,0] = 1

        for n in range(n_max):

            c[n+1,:] = torch.matmul(S_memory[n,:,:],c[n,:])

        return firing_rate.item(),c

    return firing_rate.item()
